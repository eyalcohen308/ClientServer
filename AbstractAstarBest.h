...
#ifndef CLIENTSERVER_ABSTRACTASTARBEST_H
#define CLIENTSERVER_ABSTRACTASTARBEST_H
...
protected:
    virtual void setHeuristic(State<T> *state, Searchable<T> *searchable) = 0;
...
public:
    ...
    virtual std::vector<State<T> *> search(Searchable<T> *searchable) {
         ...
         */
        this->setHeuristic(first, searchable);
        ...
        while (getOpenListSize() > 0) {
            ...
            for (typename std::vector<State<T> *>::iterator it = succerssors.begin();
                ...
                if (state_to_color.at(curr_state) == WHITE) {
                    ...
                    this->setHeuristic(curr_state, searchable);
...
#endif //CLIENTSERVER_ABSTRACTASTARBEST_H
